...
# from transformers_modules.falcon_7b.modeling_falcon import FalconDecoderLayer
from smoothquant.modeling_chatglm import RMSNorm
@torch.no_grad()
def smooth_ln_fcs(ln, fcs, act_scales, alpha=0.5):
    if not isinstance(fcs, list):
        fcs = [fcs]
    assert isinstance(ln, nn.LayerNorm) or isinstance(ln, LlamaRMSNorm) or isinstance(ln, RMSNorm) or "RMSNorm" in ln.__class__.__name__
    for fc in fcs:
...
def smooth_lm(model, scales, alpha=0.5):
    for name, module in model.named_modules():
        if isinstance(module, OPTDecoderLayer):
            ...
            # fc2 = module.mlp.down_proj
            # fc2_input_scales = scales[name + '.mlp.down_proj']
            smooth_ln_fcs(ffn_ln, fc1, fc1_input_scales, alpha)
